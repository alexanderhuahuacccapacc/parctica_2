...
public interface IUsuarioRolRepository extends ICrudGenericoRepository<UsuarioRol, UsuarioRolPK>{
    @Query("SELECT ur FROM UsuarioRol ur WHERE ur.usuario.user = :user")//Consulta JPQL // ES DECIR JPA
    List<UsuarioRol> findOneByUsuarioUser(@Param("user") String user);

    // usurio_rol es el name de la tabla pero en objetos es usuarilrol__JPA
